...
$(document).ready(function() {
    ...
    $('#simpanData').click(function() {
        var url;
        if (save_method == 'add') {
            url = "api/jenisApi.php?aksi=simpan";
        } else {
            url = "api/jenisApi.php?aksi=update";
        }
        $.ajax({
            url: url,
            type: "POST",
    ...
    $('#tabelAuthor tbody').on('click', '.tblEdit', function() {
        ...
        $.ajax({
            ...
            dataType: "JSON",
            success: function(data) {
              $('[name="x_id"]').val(data['data'][0].idJenisLayanan);
              $('[name="namajenislayanan"]').val(data['data'][0].namaJenisLayanan);
              $('#namakategori').val(data['data'][0].idJenisIdKategori).trigger('change');
              $('#modal_form').modal('show');
              $('.modal-title').html('<i class="icon icon-note"></i>	Edit Data <b>' + data['data'][0].namaJenisLayanan + '</b>');
            },
            error: function(jqXHR, textStatus, errorThrown) {
      ...
      rows_selected = table.column(0).checkboxes.selected().count();
        ...
        rows_selected = 0;
  			...
  			$('.modal-title').html('<i class="cil-trash"></i> Hapus Data Jenis Pelayanan');
  		} else {
