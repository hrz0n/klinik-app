...
$(document).ready(function() {
    ...
    table = $('#tabelAuthor').DataTable({
        ...
        "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
          table.cell( nRow, 3 ).data("Rp. "+format_uang(aData[3]));
        },
        "columnDefs": [{
                "targets": -1,
                "data": null,
    ...
    $('#simpanData').click(function() {
        var url;
        if (save_method == 'add') {
            url = "api/hargaApi.php?aksi=simpan";
        } else {
            url = "api/hargaApi.php?aksi=update";
        }
        $.ajax({
            url: url,
            type: "POST",
    ...
    $('#tabelAuthor tbody').on('click', '.tblEdit', function() {
        ...
        $.ajax({
            ...
            dataType: "JSON",
            success: function(data) {
              $('[name="x_id"]').val(data['data'][0].idHarga);
              $('[name="namapelayanan"]').val(data['data'][0].namaPelayanan);
              $('[name="harga"]').val(data['data'][0].harga);
              $('#jenislayanan').val(data['data'][0].idHargaIdJenis).trigger('change');
              $('#modal_form').modal('show');
              $('.modal-title').html('<i class="icon icon-note"></i>	Edit Data <b>' + data['data'][0].namaPelayanan + '</b>');
            },
            error: function(jqXHR, textStatus, errorThrown) {
      ...
      rows_selected = table.column(0).checkboxes.selected().count();
        ...
        rows_selected = 0;
  			...
  			$('.modal-title').html('<i class="cil-trash"></i> Hapus Data Harga Pelayanan');
  		} else {
...
function add_new() {
  ...
  $('#jenislayanan').val("").trigger('change');
    save_method = 'add';
    $('#modal_form').modal('show');
    $('.modal-title').html('<i class="icon icon-plus"></i> Tambah Data Harga Layanan');
}
