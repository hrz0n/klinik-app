...
$(document).ready(function() {
    ...
    table = $('#tabelAuthor').DataTable({
        ...
        "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
          if (aData[2] == 0) {
            table.cell( nRow, 2 ).data("Perawat");
          } else {
            table.cell( nRow, 2 ).data("Dokter");
          }
        },
        "columnDefs": [{
                "targets": -1,
    ...
    $('#simpanData').click(function() {
        var url;
        if (save_method == 'add') {
            url = "api/perawatApi.php?aksi=simpan";
        } else {
            url = "api/perawatApi.php?aksi=update";
        }
        $.ajax({
            url: url,
            type: "POST",
    ...
    $('#tabelAuthor tbody').on('click', '.tblEdit', function() {
        ...
        $.ajax({
            ...
            dataType: "JSON",
            success: function(data) {
              $('[name="x_id"]').val(data['data'][0].idPerawat);
              $('[name="namaperawat"]').val(data['data'][0].namaPerawat);
              $('[name="alamat"]').val(data['data'][0].alamat);
              $('#isdokter').val(data['data'][0].isDokter).trigger('change');
              $('#modal_form').modal('show');
              $('.modal-title').html('<i class="icon icon-note"></i>	Edit Data <b>' + data['data'][0].namaPerawat + '</b>');
            },
            error: function(jqXHR, textStatus, errorThrown) {
      ...
      rows_selected = table.column(0).checkboxes.selected().count();
        ...
        rows_selected = 0;
  			...
  			$('.modal-title').html('<i class="cil-trash"></i> Hapus Data Perawat');
  		} else {
...
function add_new() {
  ...
  $('#isdokter').val("").trigger('change');
    save_method = 'add';
    $('#modal_form').modal('show');
    $('.modal-title').html('<i class="icon icon-plus"></i> Tambah Data Perawat');
}
