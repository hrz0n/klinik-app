...
$(document).ready(function() {
    ...
    table = $('#tabelAuthor').DataTable({
        ...
        "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
          if (aData[3] == "L") {
            table.cell( nRow, 3 ).data("Laki-Laki");
          } else {
            table.cell( nRow, 3 ).data("Perempuan");
          }
        },
        "columnDefs": [{
                "targets": -1,
    ...
    $('#simpanData').click(function() {
        var url;
        if (save_method == 'add') {
            url = "api/pasienApi.php?aksi=simpan";
        } else {
            url = "api/pasienApi.php?aksi=update";
        }
        $.ajax({
            url: url,
            type: "POST",
    ...
    $('#tabelAuthor tbody').on('click', '.tblEdit', function() {
        ...
        $.ajax({
            ...
            dataType: "JSON",
            success: function(data) {
              $('[name="x_id"]').val(data['data'][0].idPasien);
              $('[name="namapasien"]').val(data['data'][0].namaPasien);
              $('[name="nrm"]').val(data['data'][0].nrmPasien);
              $('[name="nohp"]').val(data['data'][0].noHp);
              $('[name="namaortu"]').val(data['data'][0].namaOrtu);
              $('[name="alamat"]').val(data['data'][0].alamatPasien);
              $('#jk').val(data['data'][0].jenisKelamin).trigger('change');
              $('#modal_form').modal('show');
              $('.modal-title').html('<i class="icon icon-note"></i>	Edit Data <b>' + data['data'][0].namaPasien + '</b>');
            },
            error: function(jqXHR, textStatus, errorThrown) {
      ...
      rows_selected = table.column(0).checkboxes.selected().count();
        ...
        rows_selected = 0;
  			...
  			$('.modal-title').html('<i class="cil-trash"></i> Hapus Data Pasien');
  		} else {
...
function add_new() {
  ...
  $('#jk').val("").trigger('change');
    save_method = 'add';
    $('#modal_form').modal('show');
    $('.modal-title').html('<i class="icon icon-plus"></i> Tambah Data Pasien');
}
